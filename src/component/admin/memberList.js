...
import {MEMBER_ORDER} from "../../utils/constants";
...
export default function MemberList(){
    ...
    const[order,setOrder] = useState(MEMBER_ORDER.RECENT);


    function retrieveComments(num) {
    ...
    const retrievePrevPage = () => {
        if(paging.currentPage - 1 < 0)
            return;
        retrieveComments(paging.currentPage - 1);
    }

    const reloadCurrentPage = () => {
        retrieveComments(paging.currentPage);
    }

    const retrieveNextPage = () => {
        if (paging.lastPage < paging.nextPage)
            return;
        retrieveComments(paging.nextPage);
    }
            ...
            <div className={"top-space"}>
                <div className={"insite-title flex-box justify-content-center align-items-center mb-3 "}>
                    ...
                    <Form.Select onChange={orderCondChanged} className={"col-2"}>
                        <option value={MEMBER_ORDER.RECENT}>최신순</option>
                        <option value={MEMBER_ORDER.OLDER}>오래된순</option>
                        <option value={MEMBER_ORDER.QUIZ_DESC}>퀴즈 많은 순</option>
                        <option value={MEMBER_ORDER.QUIZ_ASC}>퀴즈 적은 순</option>
                        <option value={MEMBER_ORDER.COMMENT_DESC}>댓글 많은 순</option>
                        <option value={MEMBER_ORDER.COMMENT_ASC}>댓글 적은 순</option>
                    </Form.Select>
                </div>
